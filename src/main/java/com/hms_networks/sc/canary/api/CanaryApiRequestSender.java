...
public class CanaryApiRequestSender {
   ...
   * @since 1.0.0
   */
  public static boolean processRequest(String url, String headers, String body, String outputFile) {
    boolean requestSuccessful = false;
    String responseBodyString = apiRequest(url, headers, body, outputFile);
   ...
   * @since 1.0.0
   */
  public static String apiRequest(String url, String headers, String body, String outputFile) {
    String responseBodyString = "";

    try {
      responseBodyString = SCHttpUtility.httpPost(url, headers, body, outputFile);
  ...
  private static boolean checkGenericResponseJson(JSONObject response) throws JSONException {
    ...
    if (response.has(RESPONSE_STATUS_JSON_FIELD_NAME)) {
      ...
      String responseStatus = response.getString(RESPONSE_STATUS_JSON_FIELD_NAME);
      Logger.LOG_DEBUG("Response status: " + responseStatus);
    }
