...
public class SessionManager {
  ...
  private static final String RESPONSE_DIRECTORY = "/usr/responses/";
  ...
  private static final int BEFORE_EXPIRE_REFRESH_DURATION_MILIS = 8000;
  ...
  public static void refreshSessionTokens() {
    if (millisToSessionExpires() <= 0) {
      getUserToken();
      getSessionToken();
    } else if (millisToSessionExpires() < BEFORE_EXPIRE_REFRESH_DURATION_MILIS) {
      sendKeepAlive();
    }
  }
  ...
  private static boolean sendKeepAlive() {
    ...
    return CanaryApiRequestSender.processRequest(
        request.url, request.headers, request.body, request.outputFile);
  }
